import numpy as np
import torch
from D3QN_PER import PER
import gym
from torch.utils.tensorboard import SummaryWriter
from replay_buffer import Per_replay_buffer
...
parser.add_argument('--beta_incre', type=float, default=0.001)
parser.add_argument('--prio_max', type=float, default=1)
parser.add_argument('--prio_e', type=float, default=0.001)
...
agent = PER(env.observation_space.shape[0], env.action_space.n, args)

writer = SummaryWriter('../../runs/{}_PER_{}'.format(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), args.env_name))

memory = Per_replay_buffer(args.replay_size, args.alpha, args.beta, args.beta_incre, args.prio_max, args.prio_e)
