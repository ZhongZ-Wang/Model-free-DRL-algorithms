...
agent = C51(env.observation_space.shape[0], env.action_space.n, args)

writer = SummaryWriter('../../runs/{}_C51_{}'.format(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), args.env_name))
...
for t in range(args.max_timesteps):
    ...
    if len(memory) >= args.batch_size:
        loss, epsilon = agent.update_parameter(memory, args.batch_size, updates)
