...
agent = TD3(env.observation_space.shape[0], env.action_space.shape[0], args)

writer = SummaryWriter('../../runs_{}/{}_TD3_{}'.format(args.env_name, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), args.env_name))
...
for t in range(args.max_timesteps):
    ...
    if  t < args.start_steps:
        ...
        action += np.random.normal(0, args.explore_noise, size=env.action_space.shape[0]).clip(-1.0, 1.0)
