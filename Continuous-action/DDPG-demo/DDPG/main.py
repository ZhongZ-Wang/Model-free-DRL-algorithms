...
agent = DDPG(env.observation_space.shape[0], env.action_space.shape[0], args)

writer = SummaryWriter('../../runs_{}/{}_DDPG_{}'.format(args.env_name, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), args.env_name))
...
for t in range(args.max_timesteps):
    ...
    if len(memory) > args.batch_size:
        for i in range(args.update_per_step):
            critic_loss, actor_loss, target_Q, q_value = agent.update_parameter(memory, args.batch_size, updates)
            writer.add_scalar('Visualization/critic loss', critic_loss, updates)
            writer.add_scalar('Visualization/actor loss', actor_loss, updates)
            writer.add_scalar('Visualization/target q', target_Q, updates)
            writer.add_scalar('Visualization/q', q_value, updates)
            updates += 1
