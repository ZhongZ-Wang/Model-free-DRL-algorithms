...
class Agent():
    ...
    def learn(self, step, experiences, gamma, d=1):
        ...
        if FIXED_ALPHA == None:
            ...
            Q_targets = rewards.cpu() + (
                        gamma * (1 - dones.cpu()) * (Q_target_next.cpu() - FIXED_ALPHA * log_pis_next.squeeze(0).cpu()))
        # Compute critic loss
        ...
        if step % d == 0:
            ...
            if FIXED_ALPHA == None:
                ...
                actor_loss = (FIXED_ALPHA * log_pis.squeeze(0).cpu() - self.critic1(states, actions_pred.squeeze(
                    0)).cpu() - policy_prior_log_probs).mean()
            # Minimize the loss
...
if __name__ == "__main__":
    ...
    agent = Agent(state_size=state_size, action_size=action_size, random_seed=seed, hidden_size=HIDDEN_SIZE,
